...
use reqsign_azure_storage::ImdsCredentialProvider;
...
#[tokio::test]
async fn test_imds_provider() -> anyhow::Result<()> {
    if !is_test_enabled() {
        eprintln!("Skipping test: REQSIGN_AZURE_STORAGE_TEST_IMDS is not enabled");
        return Ok(());
    }

    assert_provider_reads_probe(ImdsCredentialProvider::new(), live_context()).await
}
