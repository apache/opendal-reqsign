...
async fn test_client_side_credential_access_boundary_live_interoperability() -> Result<()> {
    ...
    let body = response.text().await.map_err(|err| {
        reqsign_core::Error::unexpected("failed to read client-side CAB live response")
            .with_source(err)
    })?;
    assert!(
        status.is_success(),
    ...
    let listing: serde_json::Value = serde_json::from_str(&body).map_err(|err| {
        reqsign_core::Error::unexpected("client-side CAB live response was not valid JSON")
            .with_source(err)
    })?;
    assert!(
        listing["items"]
        ...
        "client-issued CAB token did not list the fixed probe object"
    );
