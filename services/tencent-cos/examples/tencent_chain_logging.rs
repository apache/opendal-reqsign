    ...
    async fn provide_credential(&self, ctx: &Context) -> Result<Option<Self::Credential>> {
        ...
        match self.inner.provide_credential(ctx).await {
            Ok(Some(cred)) => {
                ...
                debug!(
                    "Loaded credential with secret_id: {}...",
                    &cred.secret_id[..3.min(cred.secret_id.len())]
                );
...
async fn main() -> Result<()> {
    ...
    let chain = ProvideCredentialChain::new()
        .push(LoggingProvider::new(
            "Static",
            StaticCredentialProvider::new("demo_id", "demo_key"),
        ))
        .push(LoggingProvider::new(
            "Environment",
    ...
    match chain.provide_credential(&ctx).await? {
        Some(cred) => {
            ...
            println!("Secret ID: {}...", &cred.secret_id[..3]);
